...
class ContactController extends AdminController
{
	...
	{
		$groupes = Group::orderBy('id')
		->get()
		->pluck('name', 'id')
		->all();
		$this->setForm('group.items', $groupes);
	}
	...
	public function store (Request $request): RedirectResponse
	{
		...
		$item->groupes()->attach($item->group);
	...
	{
		$this->init();
		$this->setRule('value.unique', "contacts,value,{$id}");
		$request->validate($this->rules);

		$item = $this->model::findOrFail($id);
		$item->fill($request->all());
		$item->save();
		$item->groupes()->sync($item->group);
